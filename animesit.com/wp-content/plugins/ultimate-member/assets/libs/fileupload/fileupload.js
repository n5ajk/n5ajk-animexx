...
(function (b) {
  ...
  b.fn.uploadFile = function (t) {
    ...
    function f(w, u, v) {
      ...
      v.on("drop", function (x) {
        ...
        if (!u.multiple && s.length > 1) {
          if (u.showError) {
            b(
              "<div class='um-error-block'>" + u.multiDragErrorStr + "</div>"
            ).appendTo(w.errorLog);
          }
          return;
        }
    ...
    function l(H, B, u) {
      for (var C = 0; C < u.length; C++) {
        if (!c(B, H, u[C].name)) {
          if (H.showError) {
            b(
              "<div class='um-error-block'>" + H.extErrorStr + "</div>"
            ).appendTo(B.errorLog);
          }
          continue;
        }
        if (H.maxFileSize != -1 && u[C].size > H.maxFileSize) {
          if (H.showError) {
            b(
              "<div class='um-error-block'>" + H.sizeErrorStr + "</div>"
            ).appendTo(B.errorLog);
          }
          continue;
        }
        if (H.maxFileCount != -1 && B.selectedFiles >= H.maxFileCount) {
          if (H.showError) {
            b(
              "<div class='um-error-block'>" + H.maxFileCountErrorStr + "</div>"
            ).appendTo(B.errorLog);
          }
          continue;
        }
      ...
      z.change(function () {
        ...
        if (this.files) {
          ...
          if (!c(y, D, I)) {
            if (D.showError) {
              b(
                "<div class='um-error-block'>" + D.extErrorStr + "</div>"
              ).appendTo(y.errorLog);
            }
            return;
          }
    ...
    function p(v, u) {
      ...
      /*this.filename=b("<div class='upload-filename'></div>").appendTo(this.statusbar);*/ this.progressDiv =
        b("<div class='upload-progress'>").appendTo(this.statusbar).hide();
      this.progressbar = b(
        "<div class='upload-bar " + v.formGroup + "'></div>"
      ).appendTo(this.progressDiv);
      this.abort = b(
        "<div class='upload-red " + v.formGroup + "'>" + u.abortStr + "</div>"
      )
    ...
    function n(z, y, u, w, A) {
      ...
      var v = {
        ...
        beforeSubmit: function (F, C, E) {
          ...
          u.statusbar.append(
            "<div class='um-error-block'>" + y.uploadErrorStr + "</div>"
          );
          u.cancel.show();
          z.remove();
        ...
        error: function (C, s, B) {
          ...
          if (C.statusText == "abort") {
            ...
            if (y.showStatusAfterError) {
              u.progressDiv.hide();
              u.statusbar.append(
                "<span class='um-error-block'>ERROR: " + B + "</span>"
              );
            } else {
              u.statusbar.hide();
              u.statusbar.remove();

  ...
  $.ajaxyLiveSearch = {
    ...
    loadResults: function (object) {
      ...
      if (jQuery(elem).val() != "") {
        ...
        var loading =
          '<li class="live-search_lnk live-search_more live-search_selected">' +
          '<a id="live-search_loading" href="' +
          options.searchUrl.replace("%s", encodeURI(jQuery(elem).val())) +
        ...
        if (
          Math.ceil(containerPos.left) + parseInt(options.width, 10) >
          jQuery(window).width()
        ) {
        ...
        if (options.search) {
          ...
          m +=
            '<li class="live-search_lnk live-search_more' +
            sf_selected +
            '">{total} ' +
            "</li>";
          m = m.replace(/{search_value_escaped}/g, jQuery(elem).val());
          m = m.replace(
            /{search_url_escaped}/g,
            options.searchUrl.replace("%s", encodeURI(jQuery(elem).val()))
          );
          ...
          jQuery.post(options.ajaxUrl, data, function (resp) {
            ...
            for (var mindex in results) {
              ...
              for (var kindex in results[mindex]) {
                c = $.ajaxyLiveSearch.htmlResults(
                  results[mindex][kindex],
                  mindex,
                  kindex
                );
                m += c[0];
            ...
            if (s > 0) {
              jQuery("#live-search_val").html(
                '<ul class="live-search_main">' + m + "</ul>"
              );
            } else {
              jQuery("#live-search_val").html(
                '<ul class="live-search_main">' + m + "</ul>"
    ...
    htmlResults: function (results, type, array_index) {
      ...
      if (typeof results != "undefined") {
        if (results.all.length > 0) {
          m +=
            '<li class="live-search_header">' +
            results.title +
            '</li><li><div class="live-search_result_container"><ul>';
          for (var i = 0; i < results.all.length; i++) {
            s++;
            m +=
              "<li result-type='object' index-type='" +
              type +
              "' index-array='" +
              array_index +
              "' index='" +
    ...
    loadLiveEvents: function (object) {
      ...
      jQuery("#live-search_val li.live-search_lnk").mouseover(function () {
        jQuery(".live-search_lnk").each(function () {
          jQuery(this).attr(
            "class",
            jQuery(this).attr("class").replace(" live-search_selected", "")
          );
        });
        jQuery(this).attr(
          "class",
    ...
    loadEvents: function (object) {
        ...
        if (
          ...
          jQuery(ajaxyObject.element).attr(
            ...
            jQuery(ajaxyObject.element)
              .attr("class")
              .replace(" live-search_focused", "") + " live-search_focused"
          );
      ...
      jQuery(window).keydown(function (event) {
        if (
          jQuery("#live-search_sb").css("display") != "none" &&
          jQuery("#live-search_sb").css("display") != "undefined" &&
          ...
          if (event.keyCode == "38" || event.keyCode == "40") {
            ...
            for (var i = 0; i < all_items.length; i++) {
              if (
                jQuery(all_items[i])
                  .attr("class")
                  .indexOf("live-search_selected") >= 0 &&
                s_found == false
              ) {
                s_sel = true;
                if (i < all_items.length - 1 && event.keyCode == "40") {
                  jQuery(all_items[i]).attr(
                    "class",
                    jQuery(all_items[i])
                      .attr("class")
                      .replace(" live-search_selected", "")
                  );
                  jQuery(all_items[i + 1]).attr(
                    "class",
                    jQuery(all_items[i + 1]).attr("class") +
            ...
            if (s_sel == false) {
              if (all_items.length > 0) {
                jQuery(all_items[0]).attr(
                  "class",
                  jQuery(all_items[0]).attr("class") + " live-search_selected"
                );
              }
            ...
            var b = jQuery("#live-search_val li.live-search_selected a").attr(
              "href"
            );
            if (typeof b != "undefined" && b != "") {
              ...
              if (d.object.options.callback) {
                ...
                window.location.href = sf_url.replace(
                  "%s",
                  encodeURI(jQuery(d.object).val())
                );
              }
              return false;
            }
...
function sf_addItem(search, title, name, name_type, value) {
  ...
  if (items.length > 0) {
    for (var i = 0; i < items.length; i++) {
      if (
        jQuery(items[i])
          .find("input.live-search_ajaxy-selective-close-hidden")
          .val() == key
      ) {
        exists = true;
        break;
  ...
  var added_item = jQuery(
    '<span class="live-search_ajaxy-selective-item">' +
      title +
